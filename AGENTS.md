...
- ユーザーとの会話と解説は日本語で行う。
- 1ページにつき40問を完成させる。複数ページを連続して処理してよい。連続処理では、各ページを独立した作業単位として完成・検証・commit・pushしてから次の未完了ページへ進むこと。ユーザーが最後まで継続するよう依頼した場合は、未完了ページがなくなるまでこの手順を繰り返してよい。
...
- 完成した問題はローカルだけにためず、1ページごとに `question-sets/PAGE_NNNN.json`、`data/progress.json`、`CATALOG.md` を同じcommitでGitHubへpushする。複数ページを連続処理する場合も、複数ページ分を1つのcommitにまとめない。
